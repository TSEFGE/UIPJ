$("#idTipoArma").change(function(event){
	if(event.target.value!=""){
		$.get("/carpetas/public/carpeta/armas/"+event.target.value+"", function(response, arma){
			$("#idArma").empty();
...
$("#idDelito").change(function(event){
	if(event.target.value!=""){
		$.get("/carpetas/public/carpeta/agrupaciones1/"+event.target.value+"", function(response, delito){
			$("#idAgrupacion1").empty();
...
$("#idAgrupacion1").change(function(event){
	if(event.target.value!=""){
		$.get("/carpetas/public/carpeta/agrupaciones2/"+event.target.value+"", function(response, agrupacion1){
			$("#idAgrupacion2").empty();
			console.log(response);

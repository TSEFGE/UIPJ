$("#idClaseVehiculo").change(function(event){
	if(event.target.value!=""){
		$.get("/carpetas/public/carpeta/tipoVehiculos/"+event.target.value+"", function(response, tipo){
...
$("#idMarca").change(function(event){
	if(event.target.value!=""){
		$.get("/carpetas/public/carpeta/submarcas/"+event.target.value+"", function(response, marca){
			$("#idSubmarca").empty();
